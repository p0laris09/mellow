...
class TaskSpace {
  ...
  List<String> assignedTo; // Array of user IDs
  String createdBy; // Task creator
  String spaceId; // New field for space ID
  ...
  TaskSpace({
    ...
    required this.assignedTo, // Initialize the assignedTo field as an array
    required this.createdBy, // Initialize the createdBy field
    required this.spaceId, // Initialize the spaceId field
  ...
  bool overlapsWith(TaskSpace other) {
  ...
  String toString() {
    return 'Task(taskName: $taskName, taskType: $taskType, dueDate: $dueDate, startTime: $startTime, '
        ...
        'complexity: $complexity, weight: $weight, assignedTo: $assignedTo, createdBy: $createdBy, spaceId: $spaceId)';
  }
}

class TaskManager {
  String currentUserId;
  List<TaskSpace> tasks = [];
  ...
  Future<bool> checkTaskPreferenceInTimeSlot(TaskSpace newTask) async {
    DateTime start = newTask.startTime;
    DateTime end = newTask.endTime;

    for (String assignedUser in newTask.assignedTo) {
      ...
      // Safely access the task_preference field with a default value
      String taskPreferenceString = userDoc.data() != null
          ? (userDoc.data() as Map<String, dynamic>)['task_preference'] ??
              "4+ tasks"
          : "4+ tasks";
      ...
      while (start.isBefore(end)) {
        int taskCount = tasks
            .where((task) =>
                task.assignedTo.contains(assignedUser) &&
  ...
  Future<void> addTask(TaskSpace task) async {
  ...
  void adjustTaskDetails(TaskSpace newTask) {
    for (var task in tasks) {
      if (task.assignedTo.any((user) => newTask.assignedTo.contains(user)) &&
